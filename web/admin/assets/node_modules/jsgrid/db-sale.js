(function() {
    ...
    var db_sale = {
        ...
        loadData: function(filter) {
            return $.grep(this.sale, function(client) {
                return (!filter.Id_Sale || client.Id_Sale.indexOf(filter.Id_Sale) > -1)
                    && (!filter.Name || client.Name.indexOf(filter.Name) > -1)
                    && (!filter.Sale || client.Sale.indexOf(filter.Sale) > -1)
                    && (!filter.From || client.From.indexOf(filter.From) > -1)
                    && (!filter.To || client.To.indexOf(filter.To) > -1)
                    && (filter.Status === undefined || client.Status === filter.Status);
            });
        },

        insertItem: function(insertingProduct) {
            this.sale.push(insertingProduct);
        },

        updateItem: function(updatingProduct) { },

        deleteItem: function(deletingProduct) {
            var productIndex = $.inArray(deletingProduct, this.sale);
            this.sale.splice(productIndex, 1);
        }
    ...
    db_sale.sale = [
        {
            "Id_Sale" : "s109",
            "Name" : "Mua 2 chiếc với giá của 1",
            "Sale" : "Khuyến mãi theo đơn hàng",
            "From":"8/5/2019",
            "To": "12/12/2019",
            "Status":true
        },
