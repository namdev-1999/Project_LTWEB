...
<%
    ...
    List<Shoe> list;
    if ((List<Shoe>) session.getAttribute("result") != null) {
        list = (List<Shoe>) session.getAttribute("result");
    } else {
        list = new ArrayList<>();
    }
    ...
    if (request.getParameter("sort") != null) {
        if (request.getParameter("sort").equals("2")) {
            list = ListProduct.sortByPriceFromHighToLow((ArrayList<Shoe>) session.getAttribute("result"));
        }
        if (request.getParameter("sort").equals("1")) {
            list = ListProduct.sortByPriceFromLowToHigh((ArrayList<Shoe>) session.getAttribute("result"));
        }
    }
...
<section class="shop_grid_area section-padding-80">
    <div class="container">
        ...
        <div class="row path">
            <div class="col-lg-3 col-12 md-mt-40 sm-mt-40 bread">
                <span>  <a href="index.jsp">TRANG CHỦ</a></span>
                <span>/</span>
                <span><a class="active" href="product.jsp">TÌM KIẾM</a></span>
            </div>
        ...
        <div class="row path">
            ...
            <div class="col-8 search">
                <form class="example" action=<%=Util.fullPath("result")%>>
                    <input type="text" placeholder="Tìm kiếm theo tên, loại sản phẩm,..." name="search" id="search_input">
                    <a>
                        ...
                        <div class="col-12">
                            <div class="product-topbar d-flex align-items-center justify-content-between">
                                ...
                                <div class="product-sorting d-flex pt-2">
                                    <p>Sắp xếp:</p>
                                    <form action="<%=Util.fullPath("SortSearch")%>" method="post">
                                        <input hidden disabled value="<%=request.getParameter("search")%>">
                    ...
                    <div class="row">
                        ...
                        <div class="product product__style--3 col-lg-4 col-md-4 col-sm-6 col-12">
                            <div class="product__thumb">
                                <a class="first__img" href="detail.jsp?id=<%=s.getId()%>"><img
                            ...
                            <div class="product__content content--center">
                                <h4><a href="detail.jsp"><%=s.getName()%>
                                </a></h4>
                ...
                <nav aria-label="navigation">
                    <ul class="pagination mt-50 mb-70">
                        <li class="page-item"><a class="page-link"
                                                 href="<%=Util.fullPath("result?search=")+request.getParameter("search")%>&page=<%if (Integer.parseInt(numpage)-1 > 0){%><%=Integer.parseInt(numpage)-1 %><%}else{%><%=1%><%}%>"><i
                        ...
                        <li class="page-item"><a
                                ...
                                href="<%=Util.fullPath("result?search=")+request.getParameter("search")%>&page=<%=j%><%if(request.getParameter("sort")!=null){%>&sort=<%=request.getParameter("sort")%><%}%>"><%=j%>
                        </a></li>
                        <% }
                        %>
                        <li class="page-item"><a class="page-link"
                                                 href="<%=Util.fullPath("result?search=")+request.getParameter("search")%>&page=<%if (Integer.parseInt(numpage)+1 <= countpage){%><%=Integer.parseInt(numpage)+1 %><%}else{%><%=countpage%><%}%>"><i
